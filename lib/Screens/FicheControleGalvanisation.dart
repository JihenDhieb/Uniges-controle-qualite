...
class ControleGalvanisationScreen extends StatefulWidget {
  ...
  ControleGalvanisationScreen({Key key, String code}) : super(key: key) {
  ...
  @override
  _ControleGalvanisationScreenState createState() =>
      _ControleGalvanisationScreenState();
}

class _ControleGalvanisationScreenState
    extends State<ControleGalvanisationScreen> {
  ...
  Future<void> fetchAspects() async {
    try {
      var response = await get(Uri.parse(controleGalURL + widget.qF_Num + ",851"));

      if (response.statusCode == 200) {
        _aspects.clear();
        _aspects = aspectFromJson(response.body);
        _focusNodes.clear();
